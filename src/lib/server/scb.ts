...
export async function searchSCB(query: string): Promise<string> {
    try {
        ...
        const data = await res.json() as { tables: { id: string; label: string; updated: string; firstPeriod: string; lastPeriod: string }[] };
...
function parseJsonStat2(json: {
    ...
    const size = json.size; // e.g. [10, 1, 2]
    const value = json.value; // Flat array
    ...
    dimensions.forEach((dimId) => {
        ...
        const labels = dim.category.label; // Map of code -> label
        // We need the codes in index order
        // category.index can be an object {code: index} OR array [code] (in v2 usually object)
        const indexObj = dim.category.index;
        const codes = Object.keys(indexObj).sort((a, b) => indexObj[a] - indexObj[b]);

        categoryMap[dimId] = codes.map((c) => labels[c] || c); // Map code to label
    });

    // Helper to get coordinates
    // We strictly iterate the flat array and reconstruct coords
    const observations = [];
    ...
    for (let i = 0; i < totalCells; i++) {
        ...
        // For each dimension (in reverse specific order usually, but let's check standard)
        // JSON-stat standard: last dimension changes fastest.
        // size: [d1, d2, d3]
        // i % d3 = coord3
        // (i / d3) % d2 = coord2
        // ...
